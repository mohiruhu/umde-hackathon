...
def test_extract_best_rule_with_fallback(input_text: str, expected_keywords: List[str]) -> None:
    result = extract_rule_with_fallback(input_text)
    if expected_keywords:
        ...
        assert any(keyword in result["label"]["rule_id"] or keyword in str(result["label"]) for keyword in expected_keywords), \
            f"Expected one of {expected_keywords} in result: {result['label']}"
    else:
        assert result is None, f"Expected no rule match, got: {result}"


def test_empty_input_returns_none() -> None:
    assert extract_rule_with_fallback("") is None


def test_invalid_input_type_returns_none() -> None:
    assert extract_rule_with_fallback(None) is None  # type: ignore
    assert extract_rule_with_fallbackk(12345) is None  # type: ignore
...
def test_case_insensitivity() -> None:
    ...
    assert result is not None and "label" in result and result["label"]["rule_id"] == "TRC008"
